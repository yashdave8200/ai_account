"""
Tally ERP router — exposes endpoints for pushing transactions to Tally
and checking Tally connectivity.
"""
...
from backend.models.tally import TallyConfig, TallyPushRequest, TallyPushResponse, TallyVoucherResult
...
async def push_to_tally(request: TallyPushRequest) -> TallyPushResponse:
    ...
    Accepts a StatementData object and TallyConfig. Builds XML and POSTs
    to Tally's HTTP API. Returns a summary of created / errored vouchers.
    """
    service = TallyService(request.config)

    try:
        result: TallyVoucherResult = await service.push_statement(request.statement)
    ...
    return TallyPushResponse(status=status, message=message, result=result)


@router.post("/tally/test-push", response_model=TallyPushResponse)
async def tally_test_push(
    tally_url: str = "https://jordy-unalienable-renda.ngrok-free.dev",
    ...
    try:
        result: TallyVoucherResult = await service.push_statement(dummy_statement)
    except Exception as exc:  # noqa: BLE001
...
async def tally_health(
    ...
    try:
        ...
        return {
            "status": "unreachable",
            "tally_url": base_url,
            "error": "Connection refused — Tally is not running or the HTTP server is disabled.",
        }
